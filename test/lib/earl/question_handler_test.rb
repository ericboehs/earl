...
module Earl
  class QuestionHandlerTest < Minitest::Test
    setup do
    ...
    test "handle_tool_use with missing questions key defaults to empty array" do
      handler = build_handler
      result = handler.handle_tool_use(
        thread_id: "thread-1",
        tool_use: { id: "tu-1", name: "AskUserQuestion", input: {} }
      )

      assert_nil result
    end

    test "handle_tool_use with options that lack labels falls back to to_s" do
      posted = []
      handler = build_handler(posted: posted)
      ...
      message = posted.first[:message]
      assert_includes message, "raw-option"
    end

    test "question with no options key treats as empty array" do
      posted = []
      handler = build_handler(posted: posted)
      ...
      # Question without options still posts but with 0 reactions
      assert_equal 1, posted.size
    end
