  ...
  module Mcp
    class ApprovalHandlerTest < Minitest::Test
      ...
      test "does not auto-approve a different tool when only one is allowed" do
        ...
        request = Earl::Mcp::ApprovalHandler::ToolRequest.new(tool_name: "Edit",
                                                              input: {
                                                                "file_path" => "/tmp/foo", "new_string" => "x"
                                                              })
        handler_with_tracking.handle(request)
      ...
      test "multiple tools can be independently allowed" do
        handler = build_handler
        handler.send(:process_reaction, "white_check_mark",
                     Earl::Mcp::ApprovalHandler::ToolRequest.new(tool_name: "Bash", input: { "command" => "ls" }))
        handler.send(:process_reaction, "white_check_mark",
                     Earl::Mcp::ApprovalHandler::ToolRequest.new(tool_name: "Read", input: { "path" => "/tmp" }))
