...
module Earl
  class CommandExecutorTest < Minitest::Test
    ...
    test "!kill force kills active session" do
      ...
      assert_equal [{ signal: "KILL", pid: 99_999 }], killed
      assert_equal ["thread-1"], stopped
    ...
    test "!spawn rejects invalid directory" do
      ...
      command = Earl::CommandParser::ParsedCommand.new(name: :spawn,
                                                       args: [
                                                         "fix tests", " --dir /nonexistent/path/12345"
                                                       ])
      executor.execute(command, thread_id: "thread-1", channel_id: "channel-1")
    ...
    test "!spawn shell-escapes prompt to prevent injection" do
      ...
      command = Earl::CommandParser::ParsedCommand.new(name: :spawn,
                                                       args: [
                                                         malicious_prompt, " --name safe-test --dir /tmp"
                                                       ])
      executor.execute(command, thread_id: "thread-1", channel_id: "channel-1")
    ...
    test "!compact returns passthrough" do
      posted = []
      executor = build_executor(posted: posted)
    ...
    test "dispatch_command handles unknown command name" do
      executor = build_executor
      ctx = Earl::CommandExecutor::CommandContext.new(
        thread_id: "t-1", channel_id: "ch-1", arg: nil, args: []
      )

      # Dispatching unknown command should not raise
      assert_nothing_raised { executor.send(:dispatch_command, :nonexistent, ctx) }
    end

    test "!update calls runner.request_update when runner is present" do
      posted = []
      update_called = false
      mock_runner = Object.new
    ...
    test "!update works when runner is nil" do
      posted = []
      executor = build_executor(posted: posted, runner: nil)

      command = Earl::CommandParser::ParsedCommand.new(name: :update, args: [])
      assert_nothing_raised { executor.execute(command, thread_id: "thread-1", channel_id: "channel-1") }
      assert_includes posted.first[:message], "Updating"
    end

    test "!restart calls runner.request_restart when runner is present" do
      posted = []
    ...
    test "!restart works when runner is nil" do
      posted = []
      executor = build_executor(posted: posted, runner: nil)
    ...
    test "!escape sends SIGINT when session has a process_pid" do
      ...
      executor.define_singleton_method(:handle_escape) do |ctx|
        ...
        kill_called = true
        @deps.mattermost.create_post(**ctx.post_params(":warning: Sent SIGINT to Claude."))
      end
    ...
    test "!escape reports no session when session is nil" do
      posted = []
      executor = build_executor(posted: posted, session: nil)
    ...
    test "!kill reports no session when session is nil" do
      posted = []
      executor = build_executor(posted: posted, session: nil)
    ...
    test "!cd with empty path shows usage" do
      posted = []
      executor = build_executor(posted: posted)
    ...
    test "!cd with valid directory sets working dir" do
      posted = []
      executor = build_executor(posted: posted)
    ...
    test "!cd with nonexistent directory shows error" do
      posted = []
      executor = build_executor(posted: posted)

      command = Earl::CommandParser::ParsedCommand.new(name: :cd, args: ["/nonexistent/path/foo"])
      executor.execute(command, thread_id: "thread-1", channel_id: "channel-1")

      assert_includes posted.first[:message], "not found"
    end

    test "!stats with active session shows stats" do
      posted = []
      mock_session = Object.new
      stats = Earl::ClaudeSession::Stats.new(
        total_cost: 0.1234, total_input_tokens: 5000, total_output_tokens: 1500,
        turn_input_tokens: 1000, turn_output_tokens: 500,
      ...
      assert_equal 1, posted.size
      assert_includes posted.first[:message], "5,000"
    end

    test "!stats with persisted session shows stored stats" do
      posted = []
      executor = build_executor(posted: posted, session: nil)
      ...
      assert_equal 1, posted.size
      assert_includes posted.first[:message], "3,000"
    end

    test "!stats with no session and no persisted reports no session" do
      posted = []
      executor = build_executor(posted: posted, session: nil)
    ...
    test "!stats with persisted session with nil total_cost reports no session" do
      posted = []
      executor = build_executor(posted: posted, session: nil)
    ...
    test "format_usage omits extra section when no extra data" do
      executor = build_executor
      data = {
        "session" => { "percent_used" => 80, "resets" => "5pm" },
        "week" => { "percent_used" => 45, "resets" => "Feb 19" }
      ...
      assert_includes result, "Claude Usage"
      assert_includes result, "80% used"
      assert_not_includes result, "Extra"
    end

    test "format_usage omits extra section when extra has no percent_used" do
      executor = build_executor
      data = {
        "session" => { "percent_used" => 80, "resets" => "5pm" },
        "extra" => {}
    ...
    test "format_usage omits sections with nil percent_used" do
      executor = build_executor
      data = {
        "session" => nil,
        "week" => { "percent_used" => nil }
      }

      result = executor.send(:format_usage, data)
      assert_includes result, "Claude Usage"
      assert_not_includes result, "Session"
      assert_not_includes result, "Week"
    end

    test "format_usage includes sonnet_week section" do
      executor = build_executor
      data = {
        "sonnet_week" => { "percent_used" => 50, "resets" => "Feb 19" }
      }

      result = executor.send(:format_usage, data)
      assert_includes result, "**Sonnet:** 50% used"
    end

    test "format_context with nil categories omits category section" do
      executor = build_executor
      data = {
        "model" => "claude-opus-4-6",
        "used_tokens" => "34k",
      ...
      result = executor.send(:format_context, data)
      assert_includes result, "34k / 200k"
      assert_not_includes result, "Messages"
    end

    test "format_context category without percent" do
      executor = build_executor
      data = {
        "model" => "test",
        "used_tokens" => "10k",
      ...
      result = executor.send(:format_context, data)
      assert_includes result, "**Messages:** 5k tokens"
      # No percent in parentheses
    ...
    test "usage_fetch_body rescues StandardError" do
      ...
      executor.send(:usage_fetch_body, ctx)
      assert(posted.any? { |p| p[:message].include?("Error fetching usage") })
    end
    ...
    test "context_fetch_body rescues StandardError" do
      ...
      executor.send(:context_fetch_body, ctx, "session-123")
